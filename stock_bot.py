...
def analyze_stocks(tickers):
    ...
    for ticker in tickers:
        try:
            ...
            if change_pct > 0: change_text = f"(🔺+{change_pct:.1f}%)"
            elif change_pct < 0: change_text = f"(🟢{change_pct:.1f}%)"
            else: change_text = "(平盤)"
            ...
            if len(data) >= 120:
                sma60 = float(data['Close'].rolling(window=60).mean().iloc[-1])
                sma120 = float(data['Close'].rolling(window=120).mean().iloc[-1])
                if sma60 > sma120: score += 1
            
            if len(data) >= 6:
                avg_vol = float(data['Volume'].rolling(window=5).mean().iloc[-2])
            ...
            if score >= 2:
                light, suggest = "🟢強烈買進", f"買點參考: {min(latest_close, latest_SMA5):.2f}"
            elif score == 1:
                light, suggest = "🟡逢低試單", f"買點參考: {min(latest_close, latest_SMA5):.2f}"
            elif score < 0:
                light, suggest = "🔴獲利避險", f"賣點參考: {max(latest_close, prev_high):.2f}"
            else:
                light, suggest = "⚪繼續觀望", "建議: 多看少做"
